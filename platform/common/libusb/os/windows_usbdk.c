...
static int32_t load_usbdk_helper_dll(struct libusb_context *ctx)
{
...
static int32_t usbdk_init(struct libusb_context *ctx)
{
...
static int32_t usbdk_get_session_id_for_device(struct libusb_context *ctx,
...
static int32_t usbdk_cache_config_descriptors(struct libusb_context *ctx,
...
static inline int32_t usbdk_device_priv_init(struct libusb_context *ctx, struct libusb_device *dev, PUSB_DK_DEVICE_INFO info)
{
...
static int32_t usbdk_get_device_list(struct libusb_context *ctx, struct discovered_devs **_discdevs)
{
...
static int32_t usbdk_get_config_descriptor(struct libusb_device *dev, uint8_t config_index, void *buffer, size_t len)
{
	struct usbdk_device_priv *priv = usbi_get_device_priv(dev);
...
static int32_t usbdk_get_config_descriptor_by_value(struct libusb_device *dev, uint8_t bConfigurationValue,
	void **buffer)
{
	struct usbdk_device_priv *priv = usbi_get_device_priv(dev);
	PUSB_CONFIGURATION_DESCRIPTOR config_header;
...
static int32_t usbdk_get_active_config_descriptor(struct libusb_device *dev, void *buffer, size_t len)
{
	struct usbdk_device_priv *priv = usbi_get_device_priv(dev);
...
static int32_t usbdk_open(struct libusb_device_handle *dev_handle)
{
...
static int32_t usbdk_get_configuration(struct libusb_device_handle *dev_handle, uint8_t *config)
{
	struct usbdk_device_priv *priv = usbi_get_device_priv(dev_handle->dev);
...
static int32_t usbdk_set_configuration(struct libusb_device_handle *dev_handle, uint8_t config)
{
	UNUSED(dev_handle);
	UNUSED(config);
	return LIBUSB_SUCCESS;
...
static int32_t usbdk_claim_interface(struct libusb_device_handle *dev_handle, uint8_t iface)
{
	UNUSED(dev_handle);
	UNUSED(iface);
	return LIBUSB_SUCCESS;
...
static int32_t usbdk_set_interface_altsetting(struct libusb_device_handle *dev_handle, uint8_t iface, uint8_t altsetting)
{
	struct usbdk_device_priv *priv = usbi_get_device_priv(dev_handle->dev);
...
static int32_t usbdk_release_interface(struct libusb_device_handle *dev_handle, uint8_t iface)
{
	UNUSED(dev_handle);
	UNUSED(iface);
	return LIBUSB_SUCCESS;
...
static int32_t usbdk_clear_halt(struct libusb_device_handle *dev_handle, unsigned char endpoint)
{
	struct usbdk_device_priv *priv = usbi_get_device_priv(dev_handle->dev);
...
static int32_t usbdk_reset_device(struct libusb_device_handle *dev_handle)
{
	struct usbdk_device_priv *priv = usbi_get_device_priv(dev_handle->dev);
...
static int32_t usbdk_do_control_transfer(struct usbi_transfer *itransfer)
{
	struct libusb_transfer *transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
...
static int32_t usbdk_do_bulk_transfer(struct usbi_transfer *itransfer)
{
	struct libusb_transfer *transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
	struct usbdk_device_priv *priv = usbi_get_device_priv(transfer->dev_handle->dev);
	struct usbdk_transfer_priv *transfer_priv = get_usbdk_transfer_priv(itransfer);
...
static int32_t usbdk_do_iso_transfer(struct usbi_transfer *itransfer)
{
	struct libusb_transfer *transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
	struct usbdk_device_priv *priv = usbi_get_device_priv(transfer->dev_handle->dev);
	struct usbdk_transfer_priv *transfer_priv = get_usbdk_transfer_priv(itransfer);
...
static int32_t usbdk_submit_transfer(struct usbi_transfer *itransfer)
{
	struct libusb_transfer *transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
...
static enum libusb_transfer_status usbdk_copy_transfer_data(struct usbi_transfer *itransfer, DWORD length)
{
	...
	if (transfer->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS) {
		...
		for (i = 0; i < transfer_priv->request.IsochronousPacketsArraySize; i++) {
			...
			lib_desc->actual_length = (unsigned int32_t)transfer_priv->IsochronousResultsArray[i].ActualLength;
		}
	}

	itransfer->transferred += (int32_t)transfer_priv->request.Result.GenResult.BytesTransferred;
