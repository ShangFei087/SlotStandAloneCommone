 ...
 * DWORD/ULONG to be 'unsigned long' on 32-bit builds and 'unsigned int32_t'
...
struct winusb_device_priv {
	...
	int32_t sub_api; // for WinUSB-like APIs
	struct {
...
struct winusb_device_handle_priv {
	...
	int32_t autoclaim_count[USB_MAXINTERFACES]; // For auto-release
};
...
struct windows_backend {
	...
	int32_t (*set_configuration)(struct libusb_device_handle *dev_handle, uint8_t config);
