...
int32_t usbi_snprintf(char *dst, size_t size, const char *format, ...);
int32_t usbi_vsnprintf(char *dst, size_t size, const char *format, va_list args);
...
struct libusb_context {
	...
	void *event_data;
	unsigned int32_t event_data_cnt;
...
static inline int32_t usbi_handling_events(struct libusb_context *ctx)
{
...
struct libusb_device_handle {
	...
	int32_t auto_detach_kernel_driver;
};
...
int32_t usbi_io_init(struct libusb_context *ctx);
...
int32_t usbi_handle_transfer_completion(struct usbi_transfer *itransfer,
	enum libusb_transfer_status status);
int32_t usbi_handle_transfer_cancellation(struct usbi_transfer *itransfer);
...
int32_t usbi_add_event_source(struct libusb_context *ctx, usbi_os_handle_t os_handle,
	short poll_events);
...
int32_t usbi_create_event(usbi_event_t *event);
void usbi_destroy_event(usbi_event_t *event);
void usbi_signal_event(usbi_event_t *event);
void usbi_clear_event(usbi_event_t *event);

#ifdef HAVE_OS_TIMER
int32_t usbi_create_timer(usbi_timer_t *timer);
void usbi_destroy_timer(usbi_timer_t *timer);
int32_t usbi_arm_timer(usbi_timer_t *timer, const struct timespec *timeout);
int32_t usbi_disarm_timer(usbi_timer_t *timer);
#endif

static inline int32_t usbi_using_timer(struct libusb_context *ctx)
{
#ifdef HAVE_OS_TIMER
...
struct usbi_reported_events {
	union {
		struct {
			unsigned int32_t event_triggered:1;
#ifdef HAVE_OS_TIMER
			unsigned int32_t timer_triggered:1;
#endif
		};
		unsigned int32_t event_bits;
	};
	void *event_data;
	unsigned int32_t event_data_count;
	unsigned int32_t num_ready;
};

int32_t usbi_alloc_event_data(struct libusb_context *ctx);
int32_t usbi_wait_for_events(struct libusb_context *ctx,
	struct usbi_reported_events *reported_events, int32_t timeout_ms);
...
struct usbi_os_backend {
	 ...
	 */
	int32_t (*init)(struct libusb_context *ctx);
	 ...
	 * Return 0 on success, or a LIBUSB_ERROR code on failure.
	 */
	int32_t (*set_option)(struct libusb_context *ctx, enum libusb_option option,
		va_list args);
	 ...
	 * Return 0 on success, or a LIBUSB_ERROR code on failure.
	 */
	int32_t (*get_device_list)(struct libusb_context *ctx,
	 ...
	 * do this for you.
	 */
	int32_t (*wrap_sys_device)(struct libusb_context *ctx,
	 ...
	 * do this for you.
	 */
	int32_t (*open)(struct libusb_device_handle *dev_handle);
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*get_active_config_descriptor)(struct libusb_device *device,
	 ...
	 */
	int32_t (*get_config_descriptor)(struct libusb_device *device,
	 ...
	 */
	int32_t (*get_config_descriptor_by_value)(struct libusb_device *device,
	 ...
	 * - another LIBUSB_ERROR code on other failure.
	 */
	int32_t (*get_configuration)(struct libusb_device_handle *dev_handle, uint8_t *config);
	 ...
	 * - another LIBUSB_ERROR code on other failure.
	 */
	int32_t (*set_configuration)(struct libusb_device_handle *dev_handle, int32_t config);
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*claim_interface)(struct libusb_device_handle *dev_handle, uint8_t interface_number);
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*release_interface)(struct libusb_device_handle *dev_handle, uint8_t interface_number);
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*set_interface_altsetting)(struct libusb_device_handle *dev_handle,
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*clear_halt)(struct libusb_device_handle *dev_handle,
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*reset_device)(struct libusb_device_handle *dev_handle);

	/* Alloc num_streams usb3 bulk streams on the passed in endpoints */
	int32_t (*alloc_streams)(struct libusb_device_handle *dev_handle,
		uint32_t num_streams, unsigned char *endpoints, int32_t num_endpoints);

	/* Free usb3 bulk streams allocated with alloc_streams */
	int32_t (*free_streams)(struct libusb_device_handle *dev_handle,
	...
	int32_t (*dev_mem_free)(struct libusb_device_handle *handle, void *buffer,
		size_t len);
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*kernel_driver_active)(struct libusb_device_handle *dev_handle,
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*detach_kernel_driver)(struct libusb_device_handle *dev_handle,
		uint8_t interface_number);
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*attach_kernel_driver)(struct libusb_device_handle *dev_handle,
		uint8_t interface_number);
	 ...
	 * - another LIBUSB_ERROR code on other failure
	 */
	int32_t (*submit_transfer)(struct usbi_transfer *itransfer);
	 ...
	 * This function gets called with itransfer->lock locked!
	 */
	int32_t (*cancel_transfer)(struct usbi_transfer *itransfer);
	 ...
	 * Return 0 on success, or a LIBUSB_ERROR code on failure.
	 */
	int32_t (*handle_events)(struct libusb_context *ctx,
		void *event_data, unsigned int32_t count, unsigned int32_t num_ready);
	 ...
	 * Return 0 on success, or a LIBUSB_ERROR code on failure.
	 */
	int32_t (*handle_transfer_completion)(struct usbi_transfer *itransfer);
