...
int32_t r = libusb_bulk_transfer(dev_handle, LIBUSB_ENDPOINT_IN, data, sizeof(data), &actual_length, 0);
...
{
	int32_t *completed = transfer->user_data;
	*completed = 1;
}
...
static void calculate_timeout(struct usbi_transfer *itransfer)
{
	...
	unsigned int32_t timeout = transfer->timeout;
 ...
 */
#ifdef HAVE_OS_TIMER
static int32_t arm_timer_for_next_timeout(struct libusb_context *ctx)
{
...
#else
static inline int32_t arm_timer_for_next_timeout(struct libusb_context *ctx)
{
	UNUSED(ctx);
	return 0;
}
 ...
 * NB: flying_transfers_lock MUST be held when calling this. */
static int32_t add_to_flying_list(struct usbi_transfer *itransfer)
{
 ...
 * NB: flying_transfers_lock MUST be held when calling this. */
static int32_t remove_from_flying_list(struct usbi_transfer *itransfer)
{
	struct libusb_context *ctx = ITRANSFER_CTX(itransfer);
	int32_t rearm_timer;
	int32_t r = 0;
 ...
 */
int32_t API_EXPORTED libusb_submit_transfer(struct libusb_transfer *transfer)
{
	struct usbi_transfer *itransfer =
 ...
 */
int32_t API_EXPORTED libusb_cancel_transfer(struct libusb_transfer *transfer)
{
	struct usbi_transfer *itransfer =
		LIBUSB_TRANSFER_TO_USBI_TRANSFER(transfer);
	struct libusb_context *ctx = ITRANSFER_CTX(itransfer);
 ...
 * \ref libusb_mtasync
 */
int32_t API_EXPORTED libusb_try_lock_events(libusb_context *ctx)
{
	int32_t r;
	unsigned int32_t ru;
 ...
 */
int32_t API_EXPORTED libusb_event_handling_ok(libusb_context *ctx)
{
	unsigned int32_t r;
 ...
 * \ref libusb_mtasync
 */
int32_t API_EXPORTED libusb_event_handler_active(libusb_context *ctx)
{
	unsigned int32_t r;
 ...
 * \ref libusb_mtasync
 */
int32_t API_EXPORTED libusb_wait_for_event(libusb_context *ctx, struct timeval *tv)
{
	int32_t r;
...
static int32_t handle_event_trigger(struct libusb_context *ctx)
{
	struct list_head hotplug_msgs;
	int32_t hotplug_event = 0;
	int32_t r = 0;
...
#ifdef HAVE_OS_TIMER
static int32_t handle_timer_trigger(struct libusb_context *ctx)
{
	int32_t r;
...
static int32_t handle_events(struct libusb_context *ctx, struct timeval *tv)
{
	struct usbi_reported_events reported_events;
	int32_t r, timeout_ms;
 ...
 * and populates out
 */
static int32_t get_next_timeout(libusb_context *ctx, struct timeval *tv,
	struct timeval *out)
{
	struct timeval timeout;
	int32_t r = libusb_get_next_timeout(ctx, &timeout);
	if (r) {
 ...
 * \ref libusb_mtasync
 */
int32_t API_EXPORTED libusb_handle_events_timeout_completed(libusb_context *ctx,
 ...
 * \returns 0 on success, or a LIBUSB_ERROR code on failure
 */
int32_t API_EXPORTED libusb_handle_events_timeout(libusb_context *ctx,
	struct timeval *tv)
{
 ...
 * \returns 0 on success, or a LIBUSB_ERROR code on failure
 */
int32_t API_EXPORTED libusb_handle_events(libusb_context *ctx)
{
	struct timeval tv;
	tv.tv_sec = 60;
	tv.tv_usec = 0;
 ...
 * \ref libusb_mtasync
 */
int32_t API_EXPORTED libusb_handle_events_completed(libusb_context *ctx,
	int32_t *completed)
{
	struct timeval tv;
	tv.tv_sec = 60;
 ...
 * \ref libusb_mtasync
 */
int32_t API_EXPORTED libusb_handle_events_locked(libusb_context *ctx,
	struct timeval *tv)
{
	int32_t r;
	struct timeval poll_timeout;
 ...
 */
int32_t API_EXPORTED libusb_pollfds_handle_timeouts(libusb_context *ctx)
{
	ctx = usbi_get_context(ctx);
 ...
 */
int32_t API_EXPORTED libusb_get_next_timeout(libusb_context *ctx,
	struct timeval *tv)
{
	struct usbi_transfer *itransfer;
	struct timespec systime;
...
int32_t usbi_add_event_source(struct libusb_context *ctx, usbi_os_handle_t os_handle, short poll_events)
{
