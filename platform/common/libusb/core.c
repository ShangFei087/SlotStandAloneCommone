...
static int32_t default_context_refcnt;
...
int32_t usbi_sanitize_device(struct libusb_device *dev)
{
 ...
 */
int32_t API_EXPORTED libusb_get_port_numbers(libusb_device *dev,
	uint8_t *port_numbers, int32_t port_numbers_len)
{
	int32_t i = port_numbers_len;
	struct libusb_context *ctx = DEVICE_CTX(dev);
 ...
 */
int32_t API_EXPORTED libusb_get_port_path(libusb_context *ctx, libusb_device *dev,
 ...
 */
int32_t API_EXPORTED libusb_get_device_speed(libusb_device *dev)
{
	return dev->speed;
}
 ...
 */
int32_t API_EXPORTED libusb_get_max_packet_size(libusb_device *dev,
...
static const struct libusb_endpoint_descriptor *find_alt_endpoint(
	struct libusb_config_descriptor *config,
	int32_t iface_idx, int32_t altsetting_idx, unsigned char endpoint)
{
...
static int32_t get_endpoint_max_packet_size(libusb_device *dev,
	const struct libusb_endpoint_descriptor *ep)
{
 ...
 */
int32_t API_EXPORTED libusb_get_max_iso_packet_size(libusb_device *dev,
	unsigned char endpoint)
{
	struct libusb_config_descriptor *config;
	const struct libusb_endpoint_descriptor *ep;
 ...
 */
int32_t API_EXPORTED libusb_get_max_alt_packet_size(libusb_device *dev,
 ...
 */
int32_t API_EXPORTED libusb_wrap_sys_device(libusb_context *ctx, intptr_t sys_dev,
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int32_t API_EXPORTED libusb_open(libusb_device *dev,
	libusb_device_handle **dev_handle)
{
	struct libusb_context *ctx = DEVICE_CTX(dev);
	struct libusb_device_handle *_dev_handle;
...
void API_EXPORTED libusb_close(libusb_device_handle *dev_handle)
{
	...
	unsigned int32_t event_flags;
	int32_t handling_events;
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int32_t API_EXPORTED libusb_get_configuration(libusb_device_handle *dev_handle,
	int32_t *config)
{
	int32_t r = LIBUSB_ERROR_NOT_SUPPORTED;
	uint8_t tmp = 0;
 ...
 */
int32_t API_EXPORTED libusb_set_configuration(libusb_device_handle *dev_handle,
	int32_t configuration)
{
	usbi_dbg(HANDLE_CTX(dev_handle), "configuration %d", configuration);
	if (configuration < -1 || configuration > (int32_t)UINT8_MAX)
		return LIBUSB_ERROR_INVALID_PARAM;
 ...
 * \see libusb_set_auto_detach_kernel_driver()
 */
int32_t API_EXPORTED libusb_claim_interface(libusb_device_handle *dev_handle,
 ...
 * \see libusb_set_auto_detach_kernel_driver()
 */
int32_t API_EXPORTED libusb_release_interface(libusb_device_handle *dev_handle,
	int32_t interface_number)
{
	int32_t r;
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int32_t API_EXPORTED libusb_set_interface_alt_setting(libusb_device_handle *dev_handle,
	int32_t interface_number, int32_t alternate_setting)
{
	...
	if (interface_number < 0 || interface_number >= USB_MAXINTERFACES)
		return LIBUSB_ERROR_INVALID_PARAM;
	if (alternate_setting < 0 || alternate_setting > (int32_t)UINT8_MAX)
		return LIBUSB_ERROR_INVALID_PARAM;
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int32_t API_EXPORTED libusb_clear_halt(libusb_device_handle *dev_handle,
	unsigned char endpoint)
{
 ...
 * \returns another LIBUSB_ERROR code on other failure
 */
int32_t API_EXPORTED libusb_reset_device(libusb_device_handle *dev_handle)
{
 ...
 */
int32_t API_EXPORTED libusb_alloc_streams(libusb_device_handle *dev_handle,
 ...
 */
int32_t API_EXPORTED libusb_free_streams(libusb_device_handle *dev_handle,
 ...
 * \returns \ref LIBUSB_SUCCESS, or a LIBUSB_ERROR code on failure
 */
int32_t API_EXPORTED libusb_dev_mem_free(libusb_device_handle *dev_handle,
 ...
 */
int32_t API_EXPORTED libusb_kernel_driver_active(libusb_device_handle *dev_handle,
	int32_t interface_number)
{
	usbi_dbg(HANDLE_CTX(dev_handle), "interface %d", interface_number);
 ...
 */
int32_t API_EXPORTED libusb_detach_kernel_driver(libusb_device_handle *dev_handle,
	int32_t interface_number)
{
	usbi_dbg(HANDLE_CTX(dev_handle), "interface %d", interface_number);
 ...
 * \see libusb_kernel_driver_active()
 */
int32_t API_EXPORTED libusb_attach_kernel_driver(libusb_device_handle *dev_handle,
	int32_t interface_number)
{
	usbi_dbg(HANDLE_CTX(dev_handle), "interface %d", interface_number);
 ...
 */
int32_t API_EXPORTED libusb_set_auto_detach_kernel_driver(
 ...
 */
void API_EXPORTED libusb_set_debug(libusb_context *ctx, int32_t level)
{
	libusb_set_option(ctx, LIBUSB_OPTION_LOG_LEVEL, level);
}
 ...
 */
int32_t API_EXPORTEDV libusb_set_option(libusb_context *ctx,
	enum libusb_option option, ...)
{
	int32_t arg = 0, r = LIBUSB_SUCCESS;
	libusb_log_cb log_cb = NULL;
	va_list ap;
#if defined(ENABLE_LOGGING) && !defined(ENABLE_DEBUG_LOGGING)
	int32_t is_default_context = (NULL == ctx);
#endif
 ...
 */
int32_t API_EXPORTED libusb_init(libusb_context **ctx)
{
 ...
 * \see libusb_contexts
 */
int32_t API_EXPORTED libusb_init_context(libusb_context **ctx, const struct libusb_init_option options[], int32_t num_options)
{
 ...
 */
int32_t API_EXPORTED libusb_has_capability(uint32_t capability)
{
	switch (capability) {
...
int32_t usbi_snprintf(char *str, size_t size, const char *format, ...)
{
	va_list args;
	int32_t ret;
...
int32_t usbi_vsnprintf(char *str, size_t size, const char *format, va_list args)
{
	int32_t ret;

	ret = _vsnprintf(str, size, format, args);
	if (ret < 0 || ret == (int32_t)size) {
	...
	const char *function, const char *format, va_list args)
{
	...
	int32_t global_debug, header_len, text_len;
	static int32_t has_debug_header_been_displayed = 0;
	...
	if (header_len < 0 || header_len >= (int32_t)sizeof(buf)) {
	...
	text_len = vsnprintf(buf + header_len, sizeof(buf) - (size_t)header_len,
		format, args);
	if (text_len < 0 || text_len + header_len >= (int32_t)sizeof(buf)) {
		/* Truncated log output. On some platforms a -1 return value means
		 * that the output was truncated. */
		text_len = (int32_t)sizeof(buf) - header_len;
	}
	if (header_len + text_len + (int32_t)sizeof(USBI_LOG_LINE_END) >= (int32_t)sizeof(buf)) {
		/* Need to truncate the text slightly to fit on the terminator. */
		text_len -= (header_len + text_len + (int32_t)sizeof(USBI_LOG_LINE_END)) - (int32_t)sizeof(buf);
	}
 ...
 */
DEFAULT_VISIBILITY const char * LIBUSB_CALL libusb_error_name(int32_t error_code)
{
	switch (error_code) {
