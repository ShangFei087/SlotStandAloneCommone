...
int32_t int_vector_get(const int_vector* vec, size_t index);  // 不安全但快速的访问
int32_t int_vector_front(const int_vector* vec);
int32_t int_vector_back(const int_vector* vec);
int32_t* int_vector_data(int_vector* vec);  // 获取原始数组指针
...
int32_t int_vector_binary_search(const int_vector* vec, int32_t value);  // 假设已排序
